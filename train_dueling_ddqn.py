...
from dueling_ddqn_per_agent import Dueling_DDQN_PER_Agent
import config

def main():
    """
    Main training loop for the Dueling DDQN+PER agent playing Super Mario Bros.
    """
    print("Starting Mario Dueling DDQN+PER training...")
    ...
    model_save_path = 'mario_dueling_ddqn_per_qnet.pth'
    checkpoints_dir = 'dueling_checkpoints'
    logs_dir = 'dueling_logs'
    
    if os.path.exists(checkpoints_dir):
        checkpoint_files = [f for f in os.listdir(checkpoints_dir) if f.startswith('mario_dueling_ddqn_per_ep') and f.endswith('.pth')]
        if checkpoint_files:
            # Extract episode numbers and find the latest
            episode_nums = [int(f.split('ep')[1].split('.')[0]) for f in checkpoint_files]
            latest_episode = max(episode_nums)
            latest_checkpoint = f"{checkpoints_dir}/mario_dueling_ddqn_per_ep{latest_episode}.pth"
    ...
    for episode in progress_bar:
        ...
        if episode % config.SAVE_FREQ_EPISODE == 0:
            ...
            checkpoint_path = f"{checkpoints_dir}/mario_dueling_ddqn_per_ep{episode}.pth"
            torch.save({
                'episode': episode,
            ...
            weights_path = f"{checkpoints_dir}/mario_dueling_qnet_ep{episode}.pth"
